#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# samhati_qwen35.sh — One-command distributed Qwen3.5-9B inference
#
# Usage:
#   bash scripts/samhati_qwen35.sh              # Host: download layers 0-15, start chat
#   bash scripts/samhati_qwen35.sh <join-code>  # Join: download layers 16-31, serve
#
...
TOTAL_LAYERS=32
HALF=$((TOTAL_LAYERS / 2))   # 16
WEIGHTS=~/qwen35-9b

# Fixed gossip topic for this model network.
TOPIC="73616d6861746976317177656e332e352d39422d676f73736970746f70696330"

ARCH_ARGS="--model-type qwen35 --hidden 4096 --intermediate 12288 --heads 16 --kv-heads 4 \
  --head-dim 256 --vocab 248320 --rope-theta 10000000 --rms-eps 0.000001 \
  --partial-rotary-factor 0.25 --full-attn-interval 4 --conv-kernel 4 \
  --linear-key-heads 16 --linear-key-dim 128 --linear-value-heads 32 --linear-value-dim 128"
...
if [ -z "$JOIN_CODE" ]; then
  # ════════════════════════════════════════════════════════════════════════════
  #  HOST MODE  (Person A — serves layers 0-15, then starts interactive chat)
  # ════════════════════════════════════════════════════════════════════════════
  echo ""
  echo "┌──────────────────────────────────────────┐"
  echo "│  Samhati  ·  Qwen3.5-9B  ·  Host         │"
  echo "│  You will serve layers 0-15               │"
  echo "└──────────────────────────────────────────┘"
  echo ""
  ...
  echo "│                                                                      │"
  echo "│  Friend runs:                                                        │"
  printf "│    bash scripts/samhati_qwen35.sh %s...  │\n" "${MY_NODE_ID:0:20}"
  ...
  if [ -z "$PEER_NODE_ID" ]; then
    ...
    echo "  bash scripts/samhati_qwen35.sh $MY_NODE_ID"
    exit 1
  fi

  echo ""
  echo "════════════════════════════════════════════════════"
  echo "  Distributed Qwen3.5-9B is ready!"
  echo "  Layers  0-15 : you"
  echo "  Layers 16-31 : your friend"
  ...
  # ════════════════════════════════════════════════════════════════════════════
  #  JOIN MODE  (Person B — serves layers 16-31, bootstraps off the host)
  # ════════════════════════════════════════════════════════════════════════════
  echo ""
  echo "┌──────────────────────────────────────────┐"
  echo "│  Samhati  ·  Qwen3.5-9B  ·  Join         │"
  echo "│  You will serve layers 16-31              │"
  echo "└──────────────────────────────────────────┘"
  echo ""
  ...
  echo ""
  echo "════════════════════════════════════════════════════"
  echo "  Starting shard server (layers 16-31)..."
  echo "  Connecting to the Samhati relay network..."
